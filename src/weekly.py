...
def main(dry_run=False):
    ...
    digest = llm.complete(
        ...
        "'## Breaking News Recap' (the headline — only if any daily had breaking "
        "news), "
        "'## Work Watch — Snowflake, dbt & Cortex Code' — MY JOB DEPENDS ON THIS, "
        "so it leads everything below Breaking News whenever the week's material has "
        "ANY Snowflake, dbt, or Cortex Code (Snowflake's AI coding agent) news; group "
        "it under '### Snowflake', '### dbt', '### Cortex Code' (only groups with "
        "news), keep every citation, and do NOT repeat these items in later sections "
        "(if a Snowflake/dbt/Cortex story is itself the breaking news, cover it "
        "there); omit the section only if there is genuinely no such news this week. "
